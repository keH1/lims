<div style="display: none">
    <pre>
        <?var_dump($this->data['act_complete']['email'])?>
    </pre>
</div>
<header class="header-requirement mb-3">
	<nav class="header-menu">
		<ul class="nav">
			...
			<li class="nav-item me-1">
				<a class="nav-link <?=empty($this->data['request']['ID'])? 'disabled' : ''?>" href="<?=URI?>/request/copy/<?=$this->data['deal_id']?>" title="Скопировать">
					<svg class="icon" width="20" height="20">
	...
	<div class="panel-body">
		<div class="row mb-3">
			<div class="col-4">
				...
				<div><strong><?=$this->data['request']['PRICE']?></strong></div>
			</div>
			<div class="col-4">
		...
		<table class="table align-middle table-card">
			...
			<tr class="<?=$this->data['proposal']['check']? 'table-green' : ''?>">
				...
				<td>
					...
					<?php else: ?>
						<a class="no-decoration me-1" href="/protocol_generator/kp.php?ID=<?=$this->data['deal_id']?>&TZ_ID=<?=$this->data['request']['ID']?>" title="Сформировать">
							<svg class="icon" width="35" height="35">
								<use xlink:href="<?=URI?>/assets/images/icons.svg#form"/>
							</svg>
						</a>
			...
			<tr class="<?=$this->data['order']['check']? 'table-green' : ''?>">
				<td><strong>Договор</strong></td>
				<td>
					<?php if ($this->data['order']['is_generated']): ?>
						<a href="/contract_card.php?ID=<?=$this->data['order']['id']?>"><?=$this->data['order']['number']?></a>
					<?php else: ?>
						Не сформирован
					<?php endif; ?>
				</td>
				...
				<td>
					...
					<?php else: ?>
						<a class="no-decoration me-1" href="/protocol_generator/dogovor.php?ID=<?=$this->data['deal_id']?>&TZ_ID=<?=$this->data['tz_id']?>" title="Сформировать">
							<svg class="icon" width="35" height="35">
								<use xlink:href="<?=URI?>/assets/images/icons.svg#form"/>
							</svg>
						</a>
			...
			<tr class="<?=$this->data['attach']['check']? 'table-green' : ''?>">
				...
				<td>
					...
					<?php else: ?>
						<a class="no-decoration me-1" href="/protocol_generator/tz_doc.php?ID=<?=$this->data['deal_id']?>&TZ_ID=<?=$this->data['tz_id']?>" title="Сформировать">
							<svg class="icon" width="35" height="35">
								<use xlink:href="<?=URI?>/assets/images/icons.svg#form"/>
							</svg>
						</a>
			...
			<tr class="<?=$this->data['invoice']['check']? 'table-green' : ''?>">
				...
				<td>
					...
					<?php else: ?>
						<a class="no-decoration me-1" href="/protocol_generator/account.php?ID=<?=$this->data['deal_id']?>&TZ_ID=<?=$this->data['tz_id']?>" title="Сформировать">
							<svg class="icon" width="35" height="35">
								<use xlink:href="<?=URI?>/assets/images/icons.svg#form"/>
							</svg>
						</a>
				...
				<?php foreach ($this->data['protocol'] as $protocol): ?>
					<tr class="<?=$protocol['check']? 'table-green' : ''?>">
						...
						<td>
							...
							<?php else: ?>
								<a class="no-decoration me-1"
								   href="/protocol_generator/protocol_multiple_protocols.php?ID=<?=$this->data['deal_id']?>&TZ_ID=<?=$this->data['tz_id']?>&PROTOCOL_ID=<?=$protocol['id']?>"
								   title="Сформировать"
								>
									<svg class="icon" width="35" height="35">
										<use xlink:href="<?=URI?>/assets/images/icons.svg#form"/>
						...
						<td>
							...
							<?php else: ?>
								<a class="no-decoration me-1"
								   href="/1/1/cades_bes_file.php?ID=<?=$this->data['tz_id']?>&DATE=<?=$protocol['date']?>&NUMP=<?=$protocol['number']?>&ID_P=<?=$protocol['id']?>"
								   title="ЭЦП"
								>
									<svg class="icon" width="35" height="35">
										<use xlink:href="<?=URI?>/assets/images/icons.svg#ecp"/>
				...
				<td>
					<?php if ($this->data['is_end_test']): ?>
						<?php if ($this->data['is_may_change']): ?>
							<a class="no-decoration me-1" href="<?=URI?>/request/setStage/<?=$this->data['deal_id']?>?stage=<?=$this->data['is_complete']? '0' : '1'?>" title="Завершить испытание">
								<img src="<?=URI?>/assets/images/lockicon.png" alt="lock" width="35">
							</a>
						<?php else: ?>
							<a class="no-decoration disabled me-1" href="#" title="Завершить испытание">
								<img src="<?=URI?>/assets/images/lockicon.png" alt="lock" width="35">
							</a>
						<?php endif; ?>
					<?php else: ?>
				...
				<td class="align-top">
					<?php foreach ($this->data['file']['protocol'] as $item): ?>
						...
						<?php foreach ($item['files'] as $file): ?>
							<a href="<?=$item['dir']?><?=$file?>"><?=$file?></a><br>
						<?php endforeach; ?>
						<br>
					<?php endforeach; ?>
				</td>
...
<a href="/request_card.php?ID=<?=$this->data['deal_id']?>">Вернуться на старый дизайн</a>

<form id="act-probe-modal-form" class="bg-light mfp-hide col-md-4 m-auto p-3 position-relative" action="/add_act_num.php?id_z=<?=$this->data['deal_id']?>" method="post">
	...
	<div class="mb-3">
		...
		<select type="date" class="form-control" name="actType">
			<option value="1">Акт приемки проб</option>
			<option value="2">Контрольный лист</option>
		</select>
	</div>

	<div class="mb-3">
		<label class="form-label">Дата акта ПП</label>
		<input type="date" class="form-control" name="actDate" value="<?=date('Y-m-d')?>">
	</div>

	<div class="mb-3">
		<label class="form-label">Место отбора проб</label>
		<input type="text" class="form-control" name="samplePlace" value="<?=$this->data['sample']['place_probe']?>">
	</div>

	<div class="mb-3">
		<label class="form-label">Дата отбора проб</label>
		<input type="date" class="form-control" name="sampleDate" value="<?=$this->data['sample']['date_probe']?>">
	</div>
	<div class="mb-3">
		<label class="form-label">Описание объекта</label>
		<input type="text" class="form-control" name="DESCRIPTION" value="<?=$this->data['request']['DESCRIPTION']?>">
	</div>
	<div class="mb-3">
		<label class="form-label">Привязка к карьеру</label>
		<select type="text" class="form-control" name="quarry">
			<?php if ( empty($this->data['request']['QUARRY_ID']) ): ?>
	...
	<div class="mb-3 form-check">
		<input type="checkbox" class="form-check-input act-manual-edit" name='exampleCheck1' id="exampleCheck1" <?=$this->data['sample']['selection_type'] == 1 ? 'checked' : ''?>>
	...
	<div class="<?=$this->data['sample']['selection_type'] != 1 ? 'visually-hidden' : ''?> act-manual-block">
		<div class="mb-3">
			...
			<input type="text" class="form-control" name="sampleMaker" value="<?=$this->data['sample']['PROBE_PROIZV']?>">
		</div>
	</div>
	<div class="mb-3">
		<label class="form-label">Пробы предоставил</label>
		<input type="text" class="form-control" name="deliveryman" value="<?=$this->data['sample']['deliveryman']?>">
	</div>
