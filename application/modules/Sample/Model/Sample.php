...
class Sample extends Model
{
    ...
    public function deleteProbe($dealId, $probeId)
    {
        ...
        if ( $count == 1 ) {
            return [
                'success' => false,
                'error' => "Невозможно удалить пробу. Она единственная сохраненная у материала."
            ];
        }

        $sql = $this->DB->Query(
            "select ugtp.id, ugtp.measuring_sheet, utr.actual_value
            from ulab_gost_to_probe ugtp
            left join ulab_trial_results utr on utr.gost_to_probe_id = ugtp.id
        ...
        $gtpIds = [];
        while ($row = $sql->Fetch()) {
            $value = json_decode($row['actual_value'], true);
    ...
    {
        $sql = $this->DB->Query(
            "select mtr.id, mtr.cipher, ugtp.measuring_sheet, utr.actual_value
    ...
    public function supplementGroupMethod(array $result, $whatSupply, $supply): array
    {  
        $result[$whatSupply] = $supply;
        return $result;
    }
    ...
    {
        $data['TAKEN_SERT_ISP'] = isset($data['TAKEN_SERT_ISP'])? 1 : 0;
    ...
    public function getGostToProbe(int $ugtpId): array
    {
        ...
        $result = $this->DB->Query("SELECT * FROM ulab_gost_to_probe ugtp  WHERE id = {$ugtpId}")->Fetch();
