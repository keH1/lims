...
let protocol_id = 0 // ид протокола для подписи
let controller_url = '' // ид протокола для подписи
let file_name = '' // название файла для подписи
let fileContent = '' // Переменная для хранения информации из файла, значение присваивается в cades_bes_file.html
let global_selectbox_container = new Array();
let global_isFromCont = new Array();
...
function include_async_code()
{
    ...
    fileref.setAttribute("src", "/ulab/assets/plugins/EDS/async_code.js");
...
function Common_SignCadesBES_File(id, str) {
    var canAsync = !!cadesplugin.CreateObjectAsync;
    if (canAsync) {
        include_async_code().then(function () {
            return SignCadesBES_Async_File(id, str);
        });
    } else {
...
function MakeCadesEnhanced_NPAPI(dataToSign, tspService, certObject, sign_type) {
    ...
    if (dataToSign) {
        ...
        try {
            Signature = oSignedData.SignCades(oSigner, sign_type, true);
        }
        catch (err) {
            errormes = "Не удалось создать подпись из-за ошибки: " + cadesplugin.getLastError(err);
            alert(errormes);
...
function SignCadesBES_NPAPI_File(certListBoxId) {
    console.log('Code.js  SignCadesBES_NPAPI_File')
    var certificate = GetCertificate_NPAPI(certListBoxId);
