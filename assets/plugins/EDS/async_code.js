...
function CreateSimpleSign_Async() {
    cadesplugin.async_spawn(function* (arg) {
        ...
        try {
            ...
            SignatureFieldTitle[0].innerHTML = "Подпись сформирована успешно1:";
        } catch (err) {
...
function SignCadesBES_Async(certListBoxId, data, setDisplayData) {
    cadesplugin.async_spawn(function* (arg) {
        ...
        try {
            ...
            document.getElementById("SignatureTxtBox").innerHTML = Signature;
            SignatureFieldTitle[0].innerHTML = "Подпись сформирована успешно2:";
        } catch (err) {
            SignatureFieldTitle[0].innerHTML = "Возникла ошибка:";
            document.getElementById("SignatureTxtBox").innerHTML = err;
        }
...
function SignCadesBES_Async_File(certListBoxId, str) {
    cadesplugin.async_spawn(function* (arg) {
        var e = document.getElementById(arg[0]);
        var selectedCertID = e.selectedIndex;

        console.log('selectedIndex' + selectedCertID)
        if (selectedCertID == -1) {
        ...
        try {
            ...
            let dataToSign = fileContent; // fileContent - объявлен в Code.js
            let protocolId = protocol_id; // protocol_id - объявлен в Code.js
            let controllerUrl = controller_url; // controller_url - объявлен в Code.js
            let fileName = file_name; // controller_url - объявлен в Code.js
            ...
            $.ajax({
                ...
                method: "POST",
                dataType: 'json',
                data: {"sign": Signature, id: protocolId, file_name: fileName},
                success: function (data) {
                    console.log(data)
                    if ( data.success ) {
                        showSuccessMessage(`ЭЦП успешно создана.`)
                        SignatureFieldTitle[0].innerHTML = "Подпись сформирована успешно!";
                    } else {
                        showErrorMessage('Ошибка сохранения sig файла: ' + data.error)
                    }
                },
    ...
    cadesplugin.async_spawn(function* (arg) {
        ...
        try {
            ...
            if (dataToSign) {
                ...
                try {
                    Signature = yield oSignedData.SignCades(oSigner, sign_type), true;
                } catch (err) {
                    errormes = "Не удалось создать подпись из-за ошибки: " + cadesplugin.getLastError(err);
                    throw errormes;
                }
...
function SignCadesXML_Async(certListBoxId, signatureType) {
    cadesplugin.async_spawn(function* (arg) {
        ...
        try {
            ...
            document.getElementById("SignatureTxtBox").innerHTML = Signature;
            SignatureFieldTitle[0].innerHTML = "Подпись сформирована успешно5:";
        } catch (err) {
            SignatureFieldTitle[0].innerHTML = "Возникла ошибка:";
            document.getElementById("SignatureTxtBox").innerHTML = err;
        }
    ...
    return new Promise(function (resolve, reject) {
        cadesplugin.async_spawn(function* (args) {
            ...
            try {
                ...
                var CADES_BES = 1;
                signedMessage = yield oSignedData.SignCades(oSigner, CADES_BES, true);
