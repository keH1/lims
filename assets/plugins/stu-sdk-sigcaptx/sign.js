...
function tabletStart(id) {
    dealId = id
    var p = new WacomGSS.STU.Protocol();
    console.log('Protocol' + WacomGSS.STU)
    var intf;
    ...
    WacomGSS.STU.isDCAReady()
        ...
        .fail( function(ex) {
            ...
            if (ex instanceof DCANotReady) {
                ...
                console.error("tabletDemo failed:\n" + ex);
                setTimeout(close(), 0);
            }
        });
...
function generateImage() {
    // let downloadImage = document.getElementById("downloadImg");
    ...
    signatureCtx.strokeStyle = '#20155b';
    lastPoint = { "x": 0, "y": 0 };
    isDown = false;
    ...
    let src = signatureCanvas.toDataURL("image/png")
    saveImg(src)
}

function saveImg(src) {
    // аяксом отправить файл
    $.ajax({
        url: "/ulab/request/saveSignAjax/" + dealId,
        data: {"src": src},
        dataType: "json",
        method: "POST",
